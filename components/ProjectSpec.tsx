...
export function ProjectSpec({ project, className }: ProjectSpecProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "group rounded-xl border border-[#1f1f1f] bg-[#111111] transition-all duration-300 hover:border-[#2a2a2a] hover:shadow-[0_0_40px_rgba(74,222,128,0.03)]",
        className
      )}
    >
      {/* Accent top line */}
      ...
      <div className="p-4 sm:p-6 lg:p-7">
        ...
        {/* ── Title ───────────────────────────────────── */}
        <h3 className="mb-2 font-mono text-xl font-semibold text-[#f9fafb] transition-colors group-hover:text-[#4ade80] sm:text-2xl">
          {project.title}
        </h3>
        ...
        {/* ── Twist callout ───────────────────────────── */}
        {project.twist && (
          <div className="mb-6 flex items-start gap-3 rounded-lg border border-[#a78bfa]/15 bg-[#a78bfa]/[0.04] px-4 py-3.5">
            <svg
        ...
        {project.techStack.length > 0 && (
          <div className="mb-6 flex flex-wrap gap-2">
            {project.techStack.map((tech) => (
              <span
                key={tech}
                className="rounded-full border border-[#1f1f1f] bg-[#0a0a0a] px-2.5 py-1 font-mono text-[11px] text-[#9ca3af]"
              >
                {tech}
              </span>
            ))}
        ...
        {project.objectives.length > 0 && (
          <div className="mb-6 border-t border-[#1f1f1f] pt-5">
